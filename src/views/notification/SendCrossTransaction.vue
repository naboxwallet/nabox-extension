...
import { superLong, timesDecimals, Plus, Times, divisionDecimals, getSymbolUSD, Minus, chainToSymbol, getStorage } from "@/utils/util";
  ...
  data() {
    ...
    const validataSymbol = async (rule, value, callback) => {
      if (
        this.chain !== "NERVE" &&
        this.chain !== "NULS" &&
        !this.heterogeneousChain_In.heterogeneousChainId
      ) {
  ...
  methods: {
    ...
    async init() {
      ...
      /* const txData = {
        ...
        to: "TNVTdTSPTXQudD2FBSefpQRkXTyhhtSjyEVAF",
        data: "0x",
        value: "1",
        assetChainId: 2,
        assetId: 1,
    ...
    async getERC20Allowance() {
      ...
      if (token) {
        this.needAllowance = await this.eTransfer.getERC20Allowance(
          contractAddress,
          heterogeneousChainMultySignAddress,
    ...
    async calWithdrawalNVTFee() {
      const eTransfer = new ETransfer({
        chain: this.transferModal.network,
        network: this.network
      });
      const result = await eTransfer.calWithdrawalNVTFee(
        this.nvtUSD,
    ...
    async approveERC20() {
      ...
      const hex = await this.eTransfer.getApproveERC20Hex(
        contractAddress,
        heterogeneousChainMultySignAddress,
        this.transferModal.from
      );
