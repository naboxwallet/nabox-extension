      ...
      <div v-show="!showConfirm">
        ...
        <div class="content">
          <el-form label-position="top" :model="transferModal" :rules="transferRules" ref="transferForm">
            ...
            <el-switch class="senior_value fr" :width="32" v-model="seniorValue"
  ...
  export default {
    ...
    async created() {
      ...
      if (this.chain !== "NULS" && this.chain !== "NERVE") {
        this.eTransfer = new ETransfer({chain: this.chain, network: this.$store.state.network});
    ...
    methods: {
      ...
      async validateContractCall(sender, value, gasLimit, price, contractAddress, methodName, methodDesc, args) {
        ...
        try {
          ...
          const res = await this.$request({url: "/contract/validate/call", data: params});
          if (res.code === 1000) {
      ...
      async imputedContractCallGas(sender, value, contractAddress, methodName, methodDesc, args) {
        try {
          const params = {chain: this.chain, address: sender, value, contractAddress, methodName, methodDesc, args};
          const res = await this.$request({url: "/contract/imputed/call/gas", data: params});
          if (res.code === 1000) {
      ...
      async getContractMethodArgsTypes(contractAddress, methodName) {
        const params = {chain: this.chain, contractAddress, methodName, methodDesc: ""};
        const res = await this.$request({url: "/contract/method/args/type", data: params});
        if (res.code === 1000) {
      ...
      async broadcastTx2(hash) {
        if (!hash) return;
        const params = {chain: this.chain, hash: hash};
        const res = await this.$request({url: "/tx/transfer/save", method: "post", data: params});
        this.loading = false;
        if (res.code === 1000) {
